...
frdata$res <- residuals(lm(outcome ~ confound, data))
```

```{r freckleton_plot, out.width = '100%', out.height = 600}
...
---
class: just-img
```{r freckleton_plot2, out.width = '100%', out.height = 600}
fig <- plot_ly(frdata, x = ~ind, y = ~res, type = "scatter",
               mode = "markers")
fig <- fig %>% add_markers()
fig <- fig %>% layout(xaxis = list(title = 'Covariates'),
